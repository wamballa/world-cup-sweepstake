...
- `BL-108` `[ToDo]` `[Urgent]` `[Bug]`: Replace the inadequate daily World Cup sync with reliable automatic matchday polling every 15 to 30 minutes, keeping all football-data.org calls server-side and authenticated, respecting provider rate limits, recording and surfacing missed or failed runs, alerting when cached data becomes stale, and adding automated coverage for scheduling, sync failures, score refresh, and sweepstake recalculation. Vercel Hobby scheduling constraints require choosing and documenting either an external scheduler or a Vercel plan upgrade before implementation; keep the existing daily cron active until the replacement is deployed and verified.
